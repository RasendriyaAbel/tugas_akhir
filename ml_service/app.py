...
MODEL_DIR = Path(os.environ.get("NILM_MODEL_DIR", r"d:\cules\tugas_akhir\src\best_nilm_model (1).keras"))
_DUMMY_FILE = Path(__file__).resolve().parent / "dummy_blynk_samples.json"
_MODEL_TEXT_FILES = ("config.json", "metadata.json", "labels.json")
...
def _resolve_model_file(name: str):
  ...
  if normalized not in allowed_files:
    raise ValueError("File tidak diizinkan. Gunakan config.json, metadata.json, labels.json, atau model.weights.h5")
  return MODEL_DIR / normalized
...
def _get_model():
  ...
  try:
    _MODEL = load_model(str(MODEL_DIR), compile=False)
  except Exception as exc:
    config_path = MODEL_DIR / "config.json"
    weights_path = MODEL_DIR / "model.weights.h5"

    if not config_path.exists() or not weights_path.exists():
      raise RuntimeError(f"Gagal load model dari {MODEL_DIR}: {exc}") from exc
    ...
    try:
      ...
      raise RuntimeError(f"Gagal load model dari {MODEL_DIR}: {rebuild_exc}") from rebuild_exc
...
def _parse_sequence(payload):
  ...
  if arr.ndim == 1:
    ...
    if input_shape == [99, 8] and arr.size == 8:
      arr = arr.reshape((1, 8))
    elif input_shape == [99, 8] and arr.size == 99 * 8:
      arr = arr.reshape((99, 8))
    else:
...
def _predict_from_sequence(sequence: np.ndarray, received_len: int, payload: dict | None):
  ...
  probs = model.predict(sequence.reshape((1, 99, 8)), verbose=0)
  ...
  second_index = int(top2[1]) if len(top2) > 1 else top_index
  ...
  buffer_status = "READY" if received_len >= 99 else "LOADING"
  buffer_bar = _format_buffer_bar(received_len, 99)
  print(
...
@app.post("/reset")
def reset():
  global _EMA_PROBS, _LAST_RAW_SAMPLE, _LATEST_RESULT
  with _LOCK:
    _SEQ_BUFFER.clear()
    _EMA_PROBS = None
    _LAST_RAW_SAMPLE = None
